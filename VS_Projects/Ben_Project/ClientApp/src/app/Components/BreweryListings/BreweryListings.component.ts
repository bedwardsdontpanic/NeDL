...
async function getBreweries() {
  ...
  var toDoTable = document.getElementById('breweriesTable') as HTMLTableElement;
  toDoTable.innerHTML = "";
  for (let breweries in data) {
...
async function findDrink() {
  ...
  if (input.nodeValue != '' && input.nodeValue != null) {
    let theURL = 'https://www.thecocktaildb.com/api/json/v1/1/search.php?s=' + input.nodeValue;
    showDrink(theURL);
  }
  else {
...
async function showDrink(theURL) {
  ...
  //    console.log(data);
  for (let cocktails in data.drinks) {
    ...
    if (document.getElementById('cocktailImage') != null) {
      let theImg = document.getElementById('cocktailImage') as HTMLImageElement;
      theImg.src = data.drinks[cocktails].strDrinkThumb;
    }
