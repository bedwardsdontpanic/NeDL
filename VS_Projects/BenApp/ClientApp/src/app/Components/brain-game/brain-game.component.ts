...
export class BrainGameComponent implements OnInit {
  ...
  constructor(private http: HttpClient, @Inject('BASE_URL') baseUrl: string, @Inject(Location) private readonly location: Location) {
  ...
  public doIt(theScore: HighScore) {
    ...
    //let body = JSON.stringify({ theScore });
    //  console.log(body);
    ...
    this.http.post("https://benswebapp.azurewebsites.net/braingame", theScore, options).subscribe();
